...
public class NovelServiceImpl implements NovelService {
    ...
    @Override
    public Paging<Novel> pageQueryAll(BasePageParam param) {
        int totalCount = novelDao.queryAllBooks().size();
        List<NovelEntity> novelEntities = novelDao.pageQueryAll(param);
        List<Novel> novels = new ArrayList<>();
        if(!CollectionUtils.isEmpty(novelEntities)){
            for(NovelEntity novelEntity : novelEntities){
                novels.add(toModel(novelEntity));
            }
        }
        return new Paging<>(param.getPagination(), param.getPageSize(), totalCount, novels);
    }
